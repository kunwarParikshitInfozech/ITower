...
public class FragmentFrequentlyAlarm extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate( R.layout.fragement_frequency_alarm,
				container, false);
	...
	public void setAdapter(ListView list, BeanDashboardList data) {
		AdapterFrequentlyAlarm adapter = new AdapterFrequentlyAlarm(
				getActivity(), data);
	...
	public class AssignedTicketsTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				if(moduleUrl.equalsIgnoreCase("0")){
					url=mAppPreferences.getConfigIP()+ WebMethods.url_Get_MostFrequent_Alarms;
				}else{
					url=moduleUrl+ WebMethods.url_Get_MostFrequent_Alarms;
				}
				/*url = (moduleUrl.isEmpty()) ? Constants.url_Get_MostFrequent_Alarms
						:  "http://" + moduleUrl + "/Service.asmx/Mos_Frequent_Alarms";*/
		...
		protected void onPostExecute(Void result) {
			...
			if (!getActivity().isFinishing()) {
				...
				} else if (data_list.getMos_Frequent_Alarms_list()!=null && data_list.getMos_Frequent_Alarms_list().size() > 0) {
