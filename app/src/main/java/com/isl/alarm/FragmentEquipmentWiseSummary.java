...
public class FragmentEquipmentWiseSummary extends Fragment {
	AppPreferences mAppPreferences;
	String response_assigned_tickets;
	ListView listView;
	TextView tv_column, tv_critical, tv_major, tv_minor, tv_info;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate( R.layout.fragment_equipment_wise_summary,
				container, false);
			...
			public void onClick(View v) {
				...
				cb_critical_col.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						setAdapter(listView, datalist, a, b, c, d);
					}
				});
	...
	public void setAdapter(ListView list, BeanDashboardList data_list, int a,
                           int b, int c, int d) {
		AdapterEquipmentWiseSummary adapter = new AdapterEquipmentWiseSummary(
				getActivity(), data_list, a, b, c, d);
	...
	public class AssignedTicketsTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				if(moduleUrl.equalsIgnoreCase("0")){
					url=mAppPreferences.getConfigIP()+ WebMethods.url_Get_EquipmentwiseSummary;
				}else{
					url=moduleUrl+ WebMethods.url_Get_EquipmentwiseSummary;
				}
				/*url = (moduleUrl.isEmpty()) ? Constants.url_Get_EquipmentwiseSummary
						:  "http://" + moduleUrl + "/Service.asmx/EquipmentwiseSummary";*/
				response_assigned_tickets = Utils.httpPostRequest(con,url,
						nameValuePairs);
				Gson gson = new Gson();
				datalist = gson.fromJson(response_assigned_tickets,
		...
		protected void onPostExecute(Void result) {
			...
			if (!getActivity().isFinishing()) {
				...
				} else if (datalist.getEquipmentwiseSummary_list()!=null && datalist.getEquipmentwiseSummary_list().size() > 0) {
