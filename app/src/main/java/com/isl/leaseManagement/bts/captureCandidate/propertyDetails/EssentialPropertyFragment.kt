...
class EssentialPropertyFragment : BaseFragment() {
    ...
    private fun setupKeyboardVisibilityListener() {
        val rootView = binding.essentialPropertyCL
        ...
        rootView.viewTreeObserver.addOnGlobalLayoutListener(object :
            ...
            override fun onGlobalLayout() {
                ...
                // Ensure screenHeight is positive before using it
                val screenHeight = rootView.height
                val visibleHeight =
                    rect.height() // Get the height of the visible part of the screen
                val keypadHeight = screenHeight - visibleHeight

                // Determine if the keyboard is open based on 15% of the screen height
                val isKeyboardOpen = keypadHeight > screenHeight * 0.15

                if (wasKeyboardOpen && !isKeyboardOpen) {
                    val currentFocusView = baseActivity.currentFocus
                    currentFocusView?.clearFocus() // Clear focus from the currently focused view
                }

                wasKeyboardOpen = isKeyboardOpen // Update the state of the keyboard
            }
        })
    }
