...
class CapturedCandidateActivity : BaseActivity() {
    ...
    private fun deleteCandidatesFromPhone() {
        val autoGeneratedIds: List<Int> = candidatesListSelected
            .filter { true }  // Ensure autoGeneratedId is not null
            .map { it.autoGeneratedId }  // Extract the autoGeneratedId

        disposable =
            commonDatabase.existingCandidateListResponseItemDao()
                .deleteByAutoincrementIds(autoGeneratedIds)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ _ ->
                    reloadDataAndGetToDefaultMode()
            ...
            object : ClickInterfaces.ExistingCandidateSelection {
                ...
                override fun candidateNameSelected(selectedCandidateData: ExistingCandidateListResponse.ExistingCandidateListResponseItem) {
                    ...
                    intent.putExtra(
                        ...
                        selectedCandidateData.autoGeneratedId
                    )
                    intent.putExtra(
                    ...
                    intent.putExtra(
                        AppConstants.IntentKeys.propertyId,
                        selectedCandidateData.propertyId
                    )
                    intent.putExtra(
                        AppConstants.IntentKeys.candidateOrLandlordName,
                        selectedCandidateData.landlordName
                    )
    ...
    private fun candidateSelected(selectedCandidateData: ExistingCandidateListResponse.ExistingCandidateListResponseItem) {
        val exists =
            candidatesListSelected.any { it.autoGeneratedId == selectedCandidateData.autoGeneratedId }
        candidatesListSelected.removeAll { it.autoGeneratedId == selectedCandidateData.autoGeneratedId }
        if (!exists) {
