...
@Dao
interface CapturedCandidateListResponseItemDao {
    ...
    @Query("DELETE FROM ExistingCandidateListResponseItem WHERE autoGeneratedId IN (:autoGeneratedId)")
    fun deleteByAutoincrementIds(autoGeneratedId: List<Int>): Single<Int>

    @Query("SELECT MAX(autoGeneratedId) FROM ExistingCandidateListResponseItem")
