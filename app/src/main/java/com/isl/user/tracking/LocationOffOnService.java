...
public class LocationOffOnService extends Service {
	AppPreferences mAppPreferences;
	static String BatSta;
	DataBaseHelper dbh; 
	PackageInfo pInfo = null;
	public LocationOffOnService() {
	}
	@Override
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		mAppPreferences = new AppPreferences(LocationOffOnService.this);
		dbh = new DataBaseHelper(LocationOffOnService.this);
		dbh.open();
		try {
	...
	public class saveMobileInfo extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
		 if(res.length()!=0){
			...
			Utils.sendNotification(LocationOffOnService.this);
		  }else{
	...
	public void callWebService(){
		...
		int permission = PermissionChecker.checkSelfPermission(LocationOffOnService.this, Manifest.permission.READ_PHONE_STATE);
		if (permission == PermissionChecker.PERMISSION_GRANTED) {
			TelephonyManager telephonyManager = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
			iemi = telephonyManager.getDeviceId();
		}

		gps = new GPSTracker(LocationOffOnService.this);
		...
		try {
			...
			if(Utils.isNetworkAvailable(LocationOffOnService.this)){
				...
				obj.put("mock", mAppPreferences.getAppNameMockLocation()+"~"+gps.isMockLocation()+"~"+mAppPreferences.getLoginState());
				SignalChecker SC= new SignalChecker(LocationOffOnService.this,new SignalChecker.OnSignalReceiver() {
					@Override
			...
			if (Utils.isNetworkAvailable(LocationOffOnService.this)) {
			new saveMobileInfo(LocationOffOnService.this, obj2).execute();
		   }else{
			   stopSelf();
		   }
		  }
