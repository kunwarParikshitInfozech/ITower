...
public class MissingAsset extends Activity {
	ListView listView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.missing_asset);
		et_search=(EditText)findViewById(R.id.et_search);
		bt_search=(ImageButton)findViewById(R.id.bt_search);
		mAppPreferences = new AppPreferences(MissingAsset.this);
		...
		if (Utils.isNetworkAvailable(MissingAsset.this)) {
			new AssignedTicketsTask(MissingAsset.this).execute();
		} else {
			Toast.makeText(MissingAsset.this, "No Internet Connection",Toast.LENGTH_SHORT).show();
		}
	...
	public class AssignedTicketsTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				String response = Utils.httpPostRequest(con,mAppPreferences.getConfigIP()+ WebMethods.url_GetMissingAssetDetails, nameValuePairs);
				Gson gson = new Gson();
		...
		protected void onPostExecute(Void result) {
			...
			if (!MissingAsset.this.isFinishing()) {
				if ((data_list == null)) {
					Toast.makeText(MissingAsset.this, "Server Not Available",Toast.LENGTH_LONG).show();
				} else if (data_list.getMissing_Asset_Details_list().size() > 0) {
					listView.setVisibility(View.VISIBLE);
					ll_blank.setVisibility(View.GONE);
					setAdapter(listView, data_list);
				} else {
					Toast.makeText(MissingAsset.this, "No Data Found",Toast.LENGTH_LONG).show();
	...
	public void setAdapter(ListView list, BeanReportsList data) {
		MissingAdapter adapter = new MissingAdapter(MissingAsset.this,data);
