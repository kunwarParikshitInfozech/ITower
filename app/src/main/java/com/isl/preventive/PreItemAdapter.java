...
class PreItemAdapter extends BaseAdapter {
	...
	PreItemAdapter(Context con, List<BeanGetImage> imageList,DisplayImageOptions op) {
		this.con = con;
		this.op = op;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (imageList.get(position).getPreImgPath() != null
				&& imageList.get(position).getPreImgPath() != "") {
	        if (imageList.get(position).getPreLat() != null) {
				holder.tv_lat.setText("Latitude:"
						+ imageList.get(position).getPreLat());
			} else {
				holder.tv_lat.setText("");
			}
			if (imageList.get(position).getPreLongt() != null) {
				holder.tv_log.setText("Longitude:" + imageList.get(position).getPreLongt());
			} else {
				holder.tv_log.setText("");
			}

			if (imageList.get(position).getPreImgName() != null) {
				holder.tv_img_name.setText("Image Tag:"+ imageList.get(position).getPreImgName());
			} else {
				holder.tv_img_name.setText("");
			}
			
            if (imageList.get(position).getPreImgTimeStamp() != null) {
               holder.tv_img_time_stamp.setText("Time Stamp:"+ imageList.get(position).getPreImgTimeStamp());
               holder.tv_defalt.setBackgroundColor(Color.parseColor("#000000"));
				holder.tv_defalt.setVisibility( View.GONE );
            }else{
			   holder.tv_img_time_stamp.setText("");
			   holder.tv_defalt.setBackgroundColor(Color.parseColor("#ffffff"));
			   holder.tv_defalt.setVisibility( View.GONE );
			}
			loader.init(ImageLoaderConfiguration.createDefault(con));
			//loader.displayImage(imageList.get(position).getImageURL()+imageList.get(position).getPreImgPath(), holder.image, op, null);
			loader.displayImage(imageList.get(position).getPreImgPath(), holder.image, op, null);
			...
			holder.image.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					loader.init(ImageLoaderConfiguration.createDefault(con));
					loader.displayImage(imageList.get(position).getPreImgPath(), imageView1, op, null);
